...
| Day 6 | React dashboard and hosted API | Build passes; live inference pending |
...
- **Integration smoke test (2026-09-10).** The React build passes and five mocked Roboflow-client tests pass. No live inference request was made.

## Blockers and decisions

- Training and inference decision: use Roboflow hosted object detection because no local GPU is currently available. The React dashboard calls the FastAPI endpoint, which calls the deployed Hosted API; local/edge ONNX deployment is explicitly future work.
...
- Dashboard: Next.js React/Tailwind; FastAPI handles image preprocessing, hosted inference, filtering, and report data.
